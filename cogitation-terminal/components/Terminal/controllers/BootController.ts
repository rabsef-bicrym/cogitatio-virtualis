// cogitatio-virtualis/cogitation-terminal/components/Terminal/controllers/BootController.ts
...
export class BootController extends BaseController {
  ...
  private async runBootSequence(): Promise<void> {
    await this.enqueueOperation(async () => {
      ...
      await this.print(
        sendMessage('COGITATIO VIRTUALIS Terminal System v1.0', 'system'),
      );
      await this.delay(250);

      await this.print(
        sendMessage('Initializing boot sequence...\n', 'info'),
      );
  ...
  private async printBootMessages(
    messages: string[],
  ) {
  ...
  private async printAsciiArt(): Promise<void> {
    ...
    for (const line of COGITATIO_LOGO_LINES) {
      await this.print(
        sendLine(line.words, line.options),
      );
    }
