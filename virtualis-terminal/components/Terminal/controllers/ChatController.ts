...
export class ChatController extends BaseController {
          ...
          while (eventBoundary !== -1) {
            ...
            if (eventData) {
              try {
                ...
                console.error(
                  "[ChatController] Failed to parse SSE event:",
                  parseError,
                );
              }
            }
   ...
   */
  private async handleSSEEvent(event: ChatSSEEvent): Promise<void> {
    const { type, success, message, data } = event;
    ...
    switch (type) {
      ...
      case "complete":
        ...
        if (success) {
          ...
          if (data && data.recoverable_failure) {
            await this.print(
              sendMessage(
                "Recoverable failure occurred. Please try your command again.",
                "system",
              ),
            );
          }
      ...
      case "error":
        if (message) {
          // Process message to convert \n to actual newlines
          await this.print(
            sendMessage(`Error: ${processMessage(message)}`, "system"),
          );
        }
        break;
